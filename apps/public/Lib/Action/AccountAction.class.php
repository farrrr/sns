    ...
    public function authenticate(){
        ...
        if($verifyInfo['attach_id']){
            ...
            foreach($a as $key=>$val){
                if($val !== ""){
                    $verifyInfo['attachment'] .= D('attach')->where("attach_id=$a[$key]")->getField('name').'&nbsp;<a href="'.U('widget/Upload/down',array('attach_id'=>$a[$key])).'" target="_blank">下載</a><br />';
                }
            }
        }
        // 獲取認證分類資訊
        ...
        //附件限制
        $attach = model('Xdata')->get("admin_Config:attach");
