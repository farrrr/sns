...
class RankAction extends Action{
    ...
    public function postRank(){
        ...
        //基本排名
        $str = "我的{$typename}排行榜：粉絲數第{$ranList['followerrank']}；經驗值第{$ranList['experiencerank']}；財富值第{$ranList['scorerank']}；";

        //任務排名
        if( CheckTaskSwitch() ){
            $str .= "勳章數第{$ranList['medalrank']}；連續簽到第{$ranList['checkconrank']}；累計簽到第{$ranList['checktotalrank']}；";
        }

        $str .='快來檢視你的排名吧。'.U('public/Rank/index','type='.$type);
