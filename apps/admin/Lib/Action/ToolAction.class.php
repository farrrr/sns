...
class ToolAction extends AdministratorAction {
    ...
    public function doBackUp() {
        ...
        if (trim ( $tabledump )) {
            ...
            if (! fwrite ( $fp, $tabledump )) {
                $this->error ( '檔案目錄寫入失敗, 請檢查data目錄是否可寫' );
            } else {
                $url_param = array (
                    'filename' => $filename,
    ...
    public function doDbconvert() {
        ...
        if (false === $dao->execute ( $sql )) {
            $this->error ( '轉換ts_comment的to_uid欄位出現錯誤' );
        }
        ...
        foreach ( $apps as $k => $app ) {
            ...
            foreach ( $app_titles as $app_detail ) {
                ...
                if (! $dao->where ( "`type`='{$app}' AND `appid`={$app_detail['id']}" )->setField ( 'data', serialize ( $data ) )) {
                    echo '轉換程式出現錯誤, SQL: ' . $dao->getLastSql ();
                    exit ();
                }
    ...
    function cleancache() {
        ...
        foreach ( $dirs as $value ) {
            $this->_rmdirr ( $value );
            echo "<div style='border:2px solid green; background:#f1f1f1; padding:20px;margin:20px;width:800px;font-weight:bold;color:green;text-align:center;'>\"" . $value . "\" 檔案快取目錄已經刪除! </div> <br /><br />";
        }
        ...
        function dataDictionary() {
            ...
            foreach ( $list as $v ) {
                $default = empty ( $v ['COLUMN_DEFAULT'] ) && $v ['COLUMN_DEFAULT'] != ' ' ? '' : ' 預設值：' . $v ['COLUMN_DEFAULT'];
                $exter = $v ['EXTRA'] == 'auto_increment' ? ' 自增長欄位' : '';
