...
class IndexAction extends Action {
    ...
    public function weibaList(){
        ...
        foreach($weibaList['data'] as $k=>$v){
            $weibaList['data'][$k]['logo'] = getImageUrlByAttachIdByWeiba($v['logo']);
    ...
    public function myWeiba(){
        ...
        // if($postList['nowPage']==1){  //列表第一頁加上全局置頂的帖子
    ...
    public function detail(){
        ...
        if(!$weiba_detail){
            $this->error('該微吧不存在或已被解散');
        }
        $weiba_detail['logo'] = getImageUrlByAttachIdByWeiba($weiba_detail['logo']);
        //吧主
        ...
        if($_GET['type']!='digest' && $list['nowPage']==1){  //列表第一頁加上全局置頂的帖子
            $topPostList = D('weiba_post')->where('top=2 and is_del=0')->order('post_time desc')->findAll();  //全局置頂
    ...
    /**
     * 檢查發帖許可權
     * @return boolean 是否有發帖許可權 0：否  1：是
     */
    ...
    public function post(){
        ...
        $this->setTitle( '發表帖子 '.$weiba['weiba_name'] );
        $this->setKeywords( '發表帖子 '.$weiba['weiba_name'] );
    ...
    public function doPost(){
        ...
        if(strlen(t($_POST['title']))==0) $this->error('帖子標題不能為空');
        if(strlen(t($checkContents))==0) $this->error('帖子內容不能為空');
        preg_match_all('/./us', t($_POST['title']), $match);
        if(count($match[0])>30){     //漢字和字母都為一個字
            $this->error('帖子標題不能超過30個字');
        }
        ...
        if($res){
            ...
            return $this->ajaxReturn($res, '釋出成功', 1);
        }else{
            $this->error('釋出失敗');
        }
    }
    ...
    public function postDetail(){
        ...
        if(!$post_detail || D('weiba')->where('weiba_id='.$post_detail['weiba_id'])->getField('is_del')) $this->error('帖子不存在或已被刪除');
        if(D('weiba_favorite')->where('uid='.$this->mid.' AND post_id='.$post_id)->find()){
            $post_detail['favorite'] = 1;
        }
        //$post_detail['content'] = html_entity_decode($post_detail['content']);
    ...
    public function postEdit(){
        ...
        if( $this->mid==$post_detail['post_uid'] || CheckWeibaPermission( $weiba_admin , 0 ,'weiba_edit')){
            ...
            $this->setTitle( '編輯帖子 '.$weiba['weiba_name'] );
            $this->setKeywords( '編輯帖子 '.$weiba['weiba_name'] );
            $this->setDescription( $post_detail['title'].','.t(getShort($post_detail['content'],100)) );
            $this->display();
        }else{
            $this->error('您沒有許可權！');
    ...
    public function doPostEdit(){
        ...
        if ( !CheckWeibaPermission( '' , $weibaid ,'weiba_edit') ){
            if ( !CheckPermission('weiba_normal','weiba_edit') ){
                $this->error('對不起，您沒有許可權進行該操作！');
            }
        }
        ...
        if($res!==false){
            ...
            if(intval($_POST['log'])==1){
                D('log')->writeLog($post_detail['weiba_id'],$this->mid,'編輯了帖子“<a href="'.U('weiba/Index/postDetail',array('post_id'=>$post_id)).'" target="_blank">'.$post_detail['title'].'</a>”','posts');
            }
            //同步到微博
            ...
            return $this->ajaxReturn($post_id, '編輯成功', 1);
        }else{
            $this->error('編輯失敗');
        }
    }
    ...
    public function doReplyEdit(){
        //dump($_POST);exit;
        if(strlen(t($_POST['content']))==0) $this->error('回覆內容不能為空');
        $reply_id = intval($_POST['reply_id']);
        $data['content'] = t($_POST['content']);
        $res = D('weiba_reply')->where('reply_id='.$reply_id)->save($data);
        if($res!==false){
            return $this->ajaxReturn(intval($_POST['post_id']), '編輯成功', 1);
        }else{
            $this->error('編輯失敗');
        }
    }
    ...
    public function postDel(){
        ...
        if(D('weiba_post')->where('post_id='.$post_id)->setField('is_del',1)){
            if(intval($_POST['log'])==1){
                $post_detail = D('weiba_post')->where('post_id='.$post_id)->find();
                D('log')->writeLog($post_detail['weiba_id'],$this->mid,'刪除了帖子“'.$post_detail['title'].'”','posts');
            }
    ...
    public function postSet(){
        ...
        if(D('weiba_post')->where('post_id='.$post_id)->setField($field,$targetValue)){
            ...
            if($type==1){
                ...
                case '0':      //取消置頂
                    if($currentValue==1){
                        D('log')->writeLog($post_detail['weiba_id'],$this->mid,'將帖子“<a href="'.U('weiba/Index/postDetail',array('post_id'=>$post_id)).'" target="_blank">'.$post_detail['title'].'</a>”取消了吧內建頂','posts');
                    }else{
                        D('log')->writeLog($post_detail['weiba_id'],$this->mid,'將帖子“<a href="'.U('weiba/Index/postDetail',array('post_id'=>$post_id)).'" target="_blank">'.$post_detail['title'].'</a>”取消了全局置頂','posts');
                    }
                    ...
                    break;
                case '1':     //設為吧內建頂
                    $config['typename'] = "吧內建頂";
                    model('Notify')->sendNotify($post_detail['post_uid'], 'weiba_post_set', $config);
                    D('log')->writeLog($post_detail['weiba_id'],$this->mid,'將帖子“<a href="'.U('weiba/Index/postDetail',array('post_id'=>$post_id)).'" target="_blank">'.$post_detail['title'].'</a>”設為了吧內建頂','posts');
                    ...
                    break;
                case '2':     //設為全局置頂
                    $config['typename'] = "全局置頂";
                    model('Notify')->sendNotify($post_detail['post_uid'], 'weiba_post_set', $config);
                    D('log')->writeLog($post_detail['weiba_id'],$this->mid,'將帖子“<a href="'.U('weiba/Index/postDetail',array('post_id'=>$post_id)).'" target="_blank">'.$post_detail['title'].'</a>”設為了全局置頂','posts');
            ...
            if($type==2){
                switch ($targetValue) {
                case '0':     //取消精華
                    D('log')->writeLog($post_detail['weiba_id'],$this->mid,'將帖子“<a href="'.U('weiba/Index/postDetail',array('post_id'=>$post_id)).'" target="_blank">'.$post_detail['title'].'</a>”取消了精華','posts');
                    break;
                case '1':     //設為精華
                    $config['typename'] = "精華";
                    model('Notify')->sendNotify($post_detail['post_uid'], 'weiba_post_set', $config);
                    D('log')->writeLog($post_detail['weiba_id'],$this->mid,'將帖子“<a href="'.U('weiba/Index/postDetail',array('post_id'=>$post_id)).'" target="_blank">'.$post_detail['title'].'</a>”設為了精華','posts');
            ...
            if($type==3){
                switch ($targetValue) {
                case '0':     //取消推薦
                    D('log')->writeLog($post_detail['weiba_id'],$this->mid,'將帖子“<a href="'.U('weiba/Index/postDetail',array('post_id'=>$post_id)).'" target="_blank">'.$post_detail['title'].'</a>”取消了推薦','posts');
                    break;
                case '1':
                    //設為推薦
                    $config['typename'] = "推薦";
                    model('Notify')->sendNotify($post_detail['post_uid'], 'weiba_post_set', $config);
                    D('log')->writeLog($post_detail['weiba_id'],$this->mid,'將帖子“<a href="'.U('weiba/Index/postDetail',array('post_id'=>$post_id)).'" target="_blank">'.$post_detail['title'].'</a>”設為了推薦','posts');
    ...
    public function apply(){
        if ( !CheckPermission('weiba_normal','weiba_apply_manage') ){
            $this->error( '對不起，您沒有許可權執行該操作！' );
        }
        $weiba_id = intval($_GET['weiba_id']);
        if(D('weiba_follow')->where('weiba_id='.$weiba_id.' AND follower_uid='.$this->mid.' AND (level=3 OR level=2)')->find()){
            $this->error('您已經是吧主，不能重複申請');
        }
        if(D('weiba_post')->where('weiba_id='.$weiba_id.' AND post_uid='.$this->mid)->count()<5){
            $this->error('您需要釋出5篇以上帖子才能申請');
        }
        if(!D('weiba_follow')->where('weiba_id='.$weiba_id.' AND follower_uid='.$this->mid)->find()) $this->error('您尚未關注該微吧');
        $type = intval($_GET['type']);
        if($type!=2 && $type!=3) $this->error('參數錯誤');
    ...
    public function doApply(){
        ...
        if(strlen(t($_POST['reason']))==0) $this->error('申請理由不能為空');
        preg_match_all('/./us', t($_POST['reason']), $match);
        if(count($match[0])>140){     //漢字和字母都為一個字
            $this->error('申請理由不能超過140個字');
        }
        ...
        if($res){
            ...
            return $this->ajaxReturn($data['weiba_id'], '申請成功，請等待管理員稽覈', 1);
        }else{
            $this->error('申請失敗');
        }
    }
    ...
    /**
     * 微吧推薦
     * @param integer limit 獲取微吧條數
     * @return void
     */
    private function _weiba_recommend($limit=9){
        $weiba_recommend = D('weiba')->where('recommend=1 and is_del=0')->limit($limit)->findAll();
        foreach($weiba_recommend as $k=>$v){
            $weiba_recommend[$k]['logo'] = getImageUrlByAttachIdByWeiba($v['logo']);
        }
    ...
    private function _weibaOrder(){
        ...
        foreach($weiba_order as $k=>$v){
            $weiba_order[$k]['logo'] = getImageUrlByAttachIdByWeiba($v['logo']);
        }
