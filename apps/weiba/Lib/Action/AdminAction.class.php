...
class AdminAction extends AdministratorAction {
    ...
    public function _initialize() {
        ...
        $this->pageTitle['weibaAdminAudit'] = '吧主稽覈';
        parent::_initialize();
    }
    ...
    public function addWeiba() {
        ...
        $this->opt['who_can_post'] = array('1'=>'吧內成員','0'=>'所有人');
    ...
    public function doAddWeiba() {
        ...
        if($res) {
            if($_POST['admin_uid']){      //超級吧主加入微吧
                $follow['follower_uid'] = $data['admin_uid'];
                $follow['weiba_id'] = $res;
                $follow['level'] = 3;
                D('weiba_follow')->add($follow);
            }
            if($data['admin_uid'] != $this->mid){    //創建者加入微吧
    ...
    public function editWeiba() {
        ...
        $this->pageTab[] = array('title'=>'編輯微吧','tabHash'=>'editWeiba','url'=>U('weiba/Admin/editWeiba',array('weiba_id'=>$_GET['weiba_id'])));
    ...
    public function doEditWeiba(){
        ...
        if($res!==false) {
            ...
            if($admin_uid && $admin_uid!=$data['admin_uid']){  //如果存在吧主並且設定了新吧主，則原吧主降為普通成員
            ...
            if($data['admin_uid']){
                ...
                if(D('weiba_follow')->where($follows)->find()){  //該吧主已經為成員
    ...
    public function postList(){
        ...
        $this->pageButton[] = array('title'=>'調整回覆樓層','onclick'=>"admin.doStorey()");
        $this->pageButton[] = array('title'=>'刪除帖子','onclick'=>"admin.delPost()");
        ...
        $this->opt['top'] = array('0'=>L('PUBLIC_SYSTEMD_NOACCEPT'),'1'=>'吧內建頂','2'=>'全局置頂');
    ...
    public function postRecycle(){
        ...
        $this->_listpk = 'post_id';
        $this->pageButton[] = array('title'=>'搜索帖子','onclick'=>"admin.fold('search_form')");
        $this->pageButton[] = array('title'=>'還原','onclick'=>"admin.recoverPost()");
        $this->pageButton[] = array('title'=>'徹底刪除','onclick'=>"admin.deletePost()");
    ...
    public function editPost(){
        $this->assign('pageTitle','編輯帖子');
        // 初始化微吧列表管理選單
        $this->pageTab[] = array('title'=>'微吧列表','tabHash'=>'index','url'=>U('weiba/Admin/index'));
        $this->pageTab[] = array('title'=>'添加微吧','tabHash'=>'addWeiba','url'=>U('weiba/Admin/addWeiba'));
        $this->pageTab[] = array('title'=>'帖子列表','tabHash'=>'postList','url'=>U('weiba/Admin/postList'));
        $this->pageTab[] = array('title'=>'編輯帖子','tabHash'=>'editPost','url'=>U('weiba/Admin/editPost',array('post_id'=>$_GET['post_id'])));
        $this->pageTab[] = array('title'=>'帖子回收站','tabHash'=>'postRecycle','url'=>U('weiba/Admin/postRecycle'));
        // 列表key值 DOACTION表示操作
        ...
        $this->opt['top'] = array('0'=>L('PUBLIC_SYSTEMD_FALSE'),'1'=>'吧內建頂','2'=>'全局置頂');
    ...
    public function doStorey(){
        ...
        foreach($postList as $v){
            ...
            D('weiba_post')->where('post_id='.$v['post_id'])->setField('reply_all_count',count($replyList)); //總回覆統計數加1
        }
        echo 1;exit;
    }
    ...
    public function weibaAdminAudit(){
        ...
        // $this->pageButton[] = array('title'=>'通過','onclick'=>"admin.doAudit('', 1)");
        // $this->pageButton[] = array('title'=>'駁回','onclick'=>"admin.doAudit('', -1)");
        ...
        $listData['data'][$k]['DOACTION'] = '<a href="javascript:void(0)" onclick="admin.doAudit('.$v['weiba_id'].','.$v['follower_uid'].','.$v['type'].');">通過</a>&nbsp;|&nbsp;<a href="javascript:void(0)" onclick="admin.doAudit('.$v['weiba_id'].','.$v['follower_uid'].',-1);">駁回</a>';
        }
        ...
        private function _initWeibaListAdminMenu(){
            ...
            $this->pageTab[] = array('title'=>'帖子列表','tabHash'=>'postList','url'=>U('weiba/Admin/postList'));
            $this->pageTab[] = array('title'=>'帖子回收站','tabHash'=>'postRecycle','url'=>U('weiba/Admin/postRecycle'));
            $this->pageTab[] = array('title'=>'吧主稽覈','tabHash'=>'weibaAdminAudit','url'=>U('weiba/Admin/weibaAdminAudit'));
        }
        }
